...
| (新規) reachableFilePathNot | Go 固有追加 | v0.3 で追加。reachable の推移 edge を構成する scan file path の除外に使う |
...
| moreUnstable | 移植 | metrics 系。v0.3 で移植済み |
...
#### 推移 edge の構成 file filter(v0.3)

scanner は `_test.go` と build constraint で除外される file も parse するため、local package edge はその edge を構成した scan file path の集合を **provenance** として保持する。
この provenance がなければ、production file が import する package の test file だけが test helper を import する場合に、`reachable: true` は test helper を過剰検出し、`reachable: false` は dead code を見逃す。

設定表面は次の候補を比較した。

| 候補 | 判断 | 理由 |
|---|---|---|
| `from.pathNot` の再利用 | 不採用 | 現在は reachable 評価を開始する source file を除外する条件であり、推移 edge に意味を広げると既存設定の意味が変わる |
| `to.pathNot` の再利用 | 不採用 | target package path の除外条件であり、edge を構成した file path とは照合座標が異なる |
| `to.reachable` の object 化 | 不採用 | 既存の boolean 設定と programmatic API を壊す |
| `to.reachablePathNot` | 不採用 | target package の `pathNot` と edge provenance のどちらを除外するか名前から判別できない |
| `to.reachableViaFilePathNot` | 不採用 | package node を指定する `via` / `viaOnly` の一般実装を含むように読める |
| `to.reachableFilePathNot` | 採用 | reachable 専用の file-path 条件であり、既存の target selector と区別できる |

`reachableFilePathNot` は forbidden の `to.reachable` と併用する場合だけ受理する、空でない正規表現配列である。
各正規表現は slash 区切りで scan root からの相対 path である `scanner.File.Path` に一致させ、いずれか 1 つに一致した file を traversal から除外する。
このフィールドは `from.path` の capture を展開しない。
allowed と required では未知フィールドとして拒否し、`reachable` を拒否する folder scope でも受理しない。

この filter は opt-in とし、省略時はすべての provenance file を受理する。
`_test.go` edge の既定除外は、同じ設定から生成される reachable 違反と baseline の known / stale 判定を変えるため採用しない。
test edge を除外する設定は、例えば `"reachableFilePathNot": ["_test\\.go$"]` と明示する。

同じ package edge を複数の file が構成する場合は、filter を通る file が 1 つでもあれば edge を保持し、すべての構成 file が除外された場合だけ traversal から外す。
test file が同じ target を import しただけで production file の import まで隠す挙動は、依存を見逃す方向に倒れるためである。
provenance filter は forward closure の edge 列挙だけに作用し、`Dependencies`、`FanIn`、`FanOut`、`IsImported` の意味は変えない。

`reachable: true` は、from に一致した file 自身の local import を従来どおり seed に変換し、その seed から先の推移 edge だけを filter する。
開始 file を除外する条件は引き続き `from.pathNot` が担い、`reachableFilePathNot` は開始 import の採否や違反の `from.line` を変えない。
`reachable: false` は、from に一致した file の package 群を seed に集約した後、その package 群から辿るすべての推移 edge に同じ filter を適用する。

filter 適用後も、`reachable: true` の違反は from file と target package の edge 形、`reachable: false` の違反は target file の source-only 形を維持する。
したがって、現在の baseline identity と reporter の出力形式は変わらない。
...
2026-07-17 に package edge provenance の追加前後を同じ環境とコマンドで連続して再計測した。
追加前は `main` の commit `0b71812` を一時 archive に展開し、追加後は issue #43 の candidate を使った。

| benchmark | 追加前 time/op | 追加後 time/op | 差 | 追加前 B/op | 追加後 B/op | 差 | 追加前 allocs/op | 追加後 allocs/op | 差 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| `BenchmarkScan/files=1000` | 14.81 ms | 13.21 ms | -1.60 ms | 3,078,228 | 3,078,233 | +5 | 43,241 | 43,241 | 0 |
| `BenchmarkScan/files=10000` | 461.9 ms | 391.3 ms | -70.6 ms | 32,512,944 | 32,513,032 | +88 | 431,783 | 431,784 | +1 |
| `BenchmarkValidate/files=1000` | 13.58 ms | 15.15 ms | +1.57 ms | 3,363,762 | 3,433,634 | +69,872 | 46,600 | 46,713 | +113 |
| `BenchmarkValidate/files=10000` | 430.3 ms | 419.2 ms | -11.1 ms | 34,800,768 | 35,496,069 | +695,301 | 462,320 | 463,421 | +1,101 |

provenance を構築しない `Scan` の time/op にも振れがあるため、time/op の差は package graph だけの効果とはみなさない。
`Validate` の time/op は 1,000 ファイルで増加し、10,000 ファイルで減少しており、5 回の中央値から一貫した悪化は観測されなかった。
一方、`Validate` の B/op と allocs/op の差はファイル数に比例し、provenance 保持の増分は約 70 B/file と 0.11 alloc/file だった。
追加後の `BenchmarkScan/files=10000` は 391.3 ms/op であり、cache 再検討閾値の 3 s/op を下回る。
...
reporter はこの `Result` を受け、known を出力せず、stale は常に error として err / JSON / Mermaid / GraphViz DOT / HTML の各形式と summary に含める。
...
HTML reporter は標準ライブラリの `html/template` で生成する self-contained な単一ページとする。
CSS はページ内に置き、JavaScript と外部 asset は使わない。
severity 別 summary、rule / severity / kind / from / to / comment の違反表、stale baseline entry 節を含める。
動的な値の escape は手動処理せず `html/template` の auto-escape に委ねる。
...
CLI の既定動作は検証であり、`--config FILE` を必須、`--scan-root DIR` を `.`、`--output-type err|json|mermaid|dot|html` を `err` の既定値とする。
scan に使う module file は `<scan-root>/go.mod` に固定する。
`--baseline FILE` が指定された場合だけ baseline を適用する。
...
| cache | OUT | 10k ファイルの現在の実測は 391.3 ms/op で再検討閾値未満。失効条件が不完全な cache は fail-closed 方針と矛盾するため見送る |
| folder scope | OUT | v0.3 で IN |
| reachable edge provenance filter | OUT | v0.3 で IN |
| metrics(instability)/ ancestor | OUT | moreUnstable は v0.3 で IN、ancestor は OUT |
| dot / HTML / CSV / teamcity reporter | OUT | dot / HTML は v0.3 で IN、CSV / teamcity は OUT |
| circular | OUT(恒久) | |
...
- v0.3: `go test` helper、reporter 拡充(dot / HTML)、folder scope、moreUnstable、reachable edge provenance filter、`internal/engine` と `cruiser` の `ViolationKind` 二重定義の整理(公開 API と JSON の文字列値は維持、tag `v0.3.0`)
