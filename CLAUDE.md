...
- macOS / Linux で Claude Code / Codex の対応 `Bash` tool を実行する前に、`PreToolUse` hook が
  `make check` を直接実行する。`git push` もこの対象に含まれ、test または lint が失敗した場合は
  tool call を拒否してコマンド出力をエージェントへ返す。ネットワーク依存の `make vuln` は対象外
- `Write` / `Edit`（Codex の `apply_patch` を含む）の後は `PostToolUse` hook がリポジトリ全体へ
  `make fmt` を直接実行する
- hook は shell command の内容を解析せず、設定を単純かつ決定的に保つため、対象となるすべての
  shell call で品質ゲートを実行する
- Codex の project hook は `/hooks` からリポジトリを trust して有効化する
- この POSIX hooks は Windows、未対応の tool 経路、通常のターミナル、native Git hook には
  適用されないため、CI を最終的な品質ゲートとして維持する

## アーキテクチャ(DESIGN.ja.md の要点)

- **library-first + 薄い CLI**: エンジンは importable package として設計し、CLI は薄い wrapper。
  `cmd/godep-cruiser` は意図的に `--version` 以外の flag parsing をしていない
  (library API の形は issue ツリーで決めるため先取りしない)
...
- **baseline 3 状態セマンティクス(差別化の本体)**:
  baseline にない違反 → error、baseline にある違反 → ignore、
