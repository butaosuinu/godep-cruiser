...
| `make check` | push 前の正典となる品質ゲート。`make test` → `make lint` の順で実行 |
...
- macOS / Linux で Claude Code / Codex の対応 shell tool が `git push` を実行する前に
  `PreToolUse` hook が `make check` を実行し、test または lint が失敗した場合は push を拒否する。
  ネットワーク依存の `make vuln` は対象外
- `Write` / `Edit`（Codex の `apply_patch` を含む）の後は `PostToolUse` hook がリポジトリ全体へ
  `make fmt` を実行する
- Codex の project hook は `/hooks` からリポジトリを trust して有効化する
- Codex の `PreToolUse` が捕捉するのは対応する単純な shell 経路に限られ、別 tool 経路は迂回しうる。
  この POSIX hooks は Windows、通常のターミナル、native Git hook には適用されないため、CI を
  最終的な品質ゲートとして維持する
