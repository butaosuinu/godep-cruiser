...
**現状: v0.3 の `go test` helper、dot / HTML reporter、folder scope、moreUnstable、reachable edge provenance filter、公開型の internal alias 化まで実装完了し、`v0.3.0` としてリリース済み。**
...
- **ルール**: forbidden / allowed(fail-closed: どの allowed にもマッチしない依存は違反)/
  ...
  regex `path` / `pathNot` は配列可。
  module scope では from を file path、通常 edge の to を import path、reachable の to を
  module-relative package path に一致させる。
  folder scope では両方を module-relative package path に一致させ、いずれも from グループキャプチャ `$1` に対応する。
...
- **baseline 3 状態セマンティクス(差別化の本体)**:
  ...
  **違反が消えた baseline エントリ → stale error**(自動失効)。
  ...
  module scope の通常の edge 違反は from ファイルパス + raw import パス、reachable 違反は
  from ファイルパス + target package path、folder scope 違反は from package path + target package path、
  source-only 違反は to を省略し、いずれもルール名と組み合わせる。
...
- **reporter**: err(修正方法まで示すメッセージが設計目標)/ json / mermaid / GraphViz DOT / HTML。
