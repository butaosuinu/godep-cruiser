...
func TestContainsGitPush(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "git separate config env before push", command: "FOO=bar git --config-env foo.bar=FOO push --dry-run", want: true},
		{name: "git separate config env push alias", command: "ALIAS_VALUE=push git --config-env alias.p=ALIAS_VALUE p", want: true},
		{name: "git separate dynamic config env", command: `git --config-env "$CONFIG" p`, want: true},
		{name: "git attached dynamic config env", command: `git --config-env="$CONFIG" p`, want: true},
		...
		{name: "bash function push", command: "function publish { git push; }; publish", want: true},
		{name: "bash punctuation function push", command: "function publish-now { git push; }; publish-now", want: true},
		{name: "bash function newline before body", command: "function publish\n{ git push; }; publish", want: true},
		{name: "POSIX function push", command: "publish() { git push; }; publish", want: true},
		{name: "POSIX function newline before body", command: "publish()\n{ git push; }; publish", want: true},
		{name: "function inherits git aliases", command: "publish() { git p; }; GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=alias.p GIT_CONFIG_VALUE_0=push publish", want: true},
		{name: "chained function call push", command: "outer() { inner; }; inner() { git push; }; outer", want: true},
		{name: "function call after definition", command: "publish; publish() { git push; }; publish", want: true},
		{name: "sourced process substitution script", command: "source <(printf 'git push\\n')", want: true},
		{name: "regular sourced file fails closed", command: "source script.sh", want: true},
		{name: "dynamic sourced file fails closed", command: `source "$SCRIPT"`, want: true},
		{name: "dot sourced descriptor script", command: ". /dev/fd/3 3<<< 'git push'", want: true},
		{name: "builtin sourced descriptor script", command: "builtin source /dev/fd/3 3<<< 'git push'", want: true},
		{name: "bash environment descriptor script", command: "BASH_ENV=/dev/fd/3 bash -c ':' 3<<< 'git push'", want: true},
		{name: "env wrapped bash environment script", command: "env BASH_ENV=/dev/fd/3 bash -c ':' 3<<< 'git push'", want: true},
		{name: "split env preserves bash environment", command: "BASH_ENV=/dev/fd/3 env -S 'bash -c :' 3<<< 'git push'", want: true},
		{name: "bash environment regular file fails closed", command: "BASH_ENV=startup.sh bash -c ':'", want: true},
		{name: "bash environment dynamic file fails closed", command: `BASH_ENV="$STARTUP" bash -c ':'`, want: true},
		{name: "bash environment after noexec reset", command: "BASH_ENV=/dev/fd/3 bash -n +n -c ':' 3<<< 'git push'", want: true},
		{name: "bash environment after interactive reset", command: "BASH_ENV=/dev/fd/3 bash -i +i -c ':' 3<<< 'git push'", want: true},
		{name: "bash environment after privileged reset", command: "BASH_ENV=/dev/fd/3 bash -p +p -c ':' 3<<< 'git push'", want: true},
		{name: "bash environment after named privileged reset", command: "BASH_ENV=/dev/fd/3 bash -o privileged +o privileged -c ':' 3<<< 'git push'", want: true},
		{name: "protected bash still reads environment", command: "BASH_ENV=/dev/fd/3 bash --protected -c ':' 3<<< 'git push'", want: true},
		{name: "bash environment after POSIX reset", command: "BASH_ENV=/dev/fd/3 bash --posix +o posix -c ':' 3<<< 'git push'", want: true},
		{name: "bash environment after POSIX variable unset", command: "BASH_ENV=/dev/fd/3 POSIXLY_CORRECT=1 env -u POSIXLY_CORRECT bash -c ':' 3<<< 'git push'", want: true},
		{name: "split env unsets POSIX variable", command: "BASH_ENV=/dev/fd/3 POSIXLY_CORRECT=1 env -S '-u POSIXLY_CORRECT bash -c :' 3<<< 'git push'", want: true},
		...
		{name: "coprocess push", command: "coproc git push", want: true},
		{name: "named coprocess group push", command: "coproc worker { git push; }", want: true},
		...
		{name: "empty ANSI-C numeric escapes before push", command: `printf '%s' $'\x' $'\u' $'\U'; git push`, want: true},
		...
		{name: "unused bash function", command: "function publish { git push; }", want: false},
		{name: "unused POSIX function", command: "publish() { git push; }", want: false},
		{name: "function call before definition", command: "publish; publish() { git push; }", want: false},
		{name: "last function definition wins", command: "publish() { git push; }; publish() { git status; }; publish", want: false},
		{name: "self recursive function terminates", command: "publish() { publish; }; publish", want: false},
		...
		{name: "empty ANSI-C numeric escapes are literal", command: `printf '%s' $'\x' $'\u' $'\U'`, want: false},
		{name: "empty ANSI-C hex escape stays literal", command: `git $'\x'push`, want: false},
		{name: "empty ANSI-C short Unicode escape stays literal", command: `git $'\u'push`, want: false},
		{name: "empty ANSI-C long Unicode escape stays literal", command: `git $'\U'push`, want: false},
		...
		{name: "coprocess wraps non-push", command: "coproc git status", want: false},
		{name: "command cannot invoke coprocess reserved word", command: "command coproc git push", want: false},
		{name: "command bypasses shell function", command: "publish() { git push; }; command publish", want: false},
		...
		{name: "null sourced file is benign", command: "source /dev/null git push", want: false},
		{name: "benign sourced descriptor script", command: ". /dev/fd/3 3<<< 'git status'", want: false},
		{name: "self sourced descriptor terminates", command: ". /dev/fd/3 3<<< '. /dev/fd/3'", want: false},
		{name: "cleared bash environment is ignored", command: "BASH_ENV=/dev/fd/3 env -i bash -c ':' 3<<< 'git push'", want: false},
		{name: "unset bash environment is ignored", command: "BASH_ENV=/dev/fd/3 env -u BASH_ENV bash -c ':' 3<<< 'git push'", want: false},
		{name: "split env unsets bash environment", command: "BASH_ENV=/dev/fd/3 env -S '-u BASH_ENV bash -c :' 3<<< 'git push'", want: false},
		{name: "self referenced bash environment terminates", command: "BASH_ENV=/dev/fd/3 bash -c ':' 3<<< 'BASH_ENV=/dev/fd/3 bash -c :'", want: false},
		{name: "benign bash environment descriptor", command: "BASH_ENV=/dev/fd/3 bash -c ':' 3<<< 'git status'", want: false},
		{name: "empty bash environment is ignored", command: "BASH_ENV= bash -c ':'", want: false},
		{name: "null bash environment is ignored", command: "BASH_ENV=/dev/null bash -c ':'", want: false},
		{name: "noexec ignores bash environment", command: "BASH_ENV=/dev/fd/3 bash -n -c ':' 3<<< 'git push'", want: false},
		{name: "interactive bash ignores bash environment", command: "BASH_ENV=/dev/fd/3 bash -i -c ':' 3<<< 'git push'", want: false},
		{name: "privileged bash ignores bash environment", command: "BASH_ENV=/dev/fd/3 bash -p -c ':' 3<<< 'git push'", want: false},
		{name: "named privileged bash ignores environment", command: "BASH_ENV=/dev/fd/3 bash -o privileged -c ':' 3<<< 'git push'", want: false},
		{name: "POSIX bash ignores bash environment", command: "BASH_ENV=/dev/fd/3 bash --posix -c ':' 3<<< 'git push'", want: false},
		{name: "named POSIX bash ignores environment", command: "BASH_ENV=/dev/fd/3 bash -o posix -c ':' 3<<< 'git push'", want: false},
		{name: "POSIX environment variable ignores bash environment", command: "BASH_ENV=/dev/fd/3 POSIXLY_CORRECT= bash -c ':' 3<<< 'git push'", want: false},
		{name: "other shell ignores bash environment", command: "BASH_ENV=/dev/fd/3 sh -c ':' 3<<< 'git push'", want: false},
		...
		{name: "git separate config env non-push", command: "FOO=bar git --config-env foo.bar=FOO status", want: false},
		{name: "git separate config env missing value", command: "git --config-env push", want: false},
